...
class VerdictHandler:
    def raise_if_blocked(self, verdict: Verdict, event_name: str) -> None:
        if verdict.decision == Decision.DENY:
            logger.warning(f"Policy denied at {event_name}: {verdict.reasoning}")
            raise PolicyDenied(verdict)

        if verdict.decision == Decision.ESCALATE:
            logger.info(f"Policy escalation at {event_name}: {verdict.reasoning}")
