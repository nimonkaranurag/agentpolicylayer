...
class LangGraphStateExtractor:
    ...
    def extract_messages(self, state: Any) -> list[Message]:
        raw_messages = self._get_raw_messages_from_state(state)
        if not raw_messages:
            return []
        return self._message_adapter.to_apl_messages(raw_messages)

    def extract_metadata(self, state: Any, config: dict | None) -> SessionMetadata:
        ...
        return SessionMetadata(session_id=session_id, user_id=user_id)

    def _get_raw_messages_from_state(self, state: Any) -> list | None:
