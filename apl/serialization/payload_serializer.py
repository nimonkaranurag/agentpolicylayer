...
class PayloadSerializer:
    ...
    def serialize(self, payload: EventPayload) -> dict[str, Any]:
        ...
        if payload.llm_prompt is not None and self._message_serializer:
            result["llm_prompt"] = [
                self._message_serializer.serialize(m) for m in payload.llm_prompt
            ]
    ...
    def deserialize(self, data: dict[str, Any]) -> EventPayload:
        return EventPayload(
            ...
            llm_response=(
                self._message_serializer.deserialize(data["llm_response"])
            ...
            llm_prompt=(
                [self._message_serializer.deserialize(m) for m in data["llm_prompt"]]
                if data.get("llm_prompt") and self._message_serializer
