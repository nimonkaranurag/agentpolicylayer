...
class MessageSerializer:
    def serialize(self, message: Message) -> dict[str, Any]:
        ...
        if message.tool_calls is not None:
            result["tool_calls"] = [
                self._serialize_tool_call(tc) for tc in message.tool_calls
            ]
    ...
    def deserialize(self, data: dict[str, Any]) -> Message:
        ...
        if data.get("tool_calls"):
            tool_calls = [self._deserialize_tool_call(tc) for tc in data["tool_calls"]]
    ...
    def _deserialize_tool_call(self, data: dict[str, Any]) -> ToolCall:
        return ToolCall(
            id=data["id"],
