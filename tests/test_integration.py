...
class TestEndToEndPolicyEvaluation:
    ...
    async def test_full_server_evaluation_flow(self, make_event):
        ...
        @server.policy(name="pii-filter", events=["output.pre_send"])
        ...
        @server.policy(name="length-check", events=["output.pre_send"])
    ...
    async def test_composition_after_evaluation(self, make_event):
        ...
        deny_composer = VerdictComposer(CompositionConfig(mode=CompositionMode.DENY_OVERRIDES))
        result = deny_composer.compose(verdicts)
        assert result.decision == Decision.DENY

        allow_composer = VerdictComposer(CompositionConfig(mode=CompositionMode.ALLOW_OVERRIDES))
...
class TestEndToEndSerialization:
    ...
    def test_event_serialize_deserialize_with_full_data(self):
...
class TestEndToEndDeclarativeRules:
    ...
    def test_multi_rule_first_match_wins(self):
        ...
        builder = __import__("apl.layer.event_builder", fromlist=["PolicyEventBuilder"]).PolicyEventBuilder()

        rules = [
            YAMLRule(
                when={"payload.output_text": {"contains": "SECRET"}},
                then={"decision": "deny", "reasoning": "secret detected"},
            ),
            YAMLRule(
                when={"metadata.user_region": "EU"},
                then={"decision": "modify", "reasoning": "GDPR compliance",
                       "modification": {"target": "output", "operation": "redact", "value": "[REDACTED]"}},
            ),
            YAMLRule(
                when={},
                then={"decision": "allow"},
