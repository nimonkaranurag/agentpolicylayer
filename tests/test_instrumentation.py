...
from apl.instrumentation.events.agent_post_handoff_event import AgentPostHandoffEvent
from apl.instrumentation.events.agent_pre_handoff_event import AgentPreHandoffEvent
from apl.instrumentation.events.base_event import BaseEvent
from apl.instrumentation.events.input_received_event import InputReceivedEvent
from apl.instrumentation.events.input_validated_event import InputValidatedEvent
from apl.instrumentation.events.llm_post_response_event import LLMPostResponseEvent
from apl.instrumentation.events.llm_pre_request_event import LLMPreRequestEvent
from apl.instrumentation.events.output_pre_send_event import OutputPreSendEvent
from apl.instrumentation.events.plan_approved_event import PlanApprovedEvent
from apl.instrumentation.events.plan_proposed_event import PlanProposedEvent
from apl.instrumentation.events.session_end_event import SessionEndEvent
from apl.instrumentation.events.session_start_event import SessionStartEvent
from apl.instrumentation.events.tool_post_invoke_event import ToolPostInvokeEvent
from apl.instrumentation.events.tool_pre_invoke_event import ToolPreInvokeEvent
...
from apl.types import Decision, EventPayload, EventType, Modification, Verdict
...
class TestEventRegistry:
    ...
    def test_get_event_returns_correct_type(self):
        assert isinstance(get_event("session.start"), SessionStartEvent)
        assert isinstance(get_event("output.pre_send"), OutputPreSendEvent)
        assert isinstance(get_event("tool.pre_invoke"), ToolPreInvokeEvent)
...
class TestApplyVerdictModifications:
    ...
    def test_modify_with_no_modification_is_noop(self):
        event = SessionStartEvent()
        verdict = Verdict(decision=Decision.MODIFY, modification=None)
        ctx = type("ctx", (), {})()
        event.apply_verdict_modifications(verdict, ctx)
...
class TestExecutorInheritance:
    ...
    def test_sync_executor_inherits_evaluator(self):
        assert issubclass(SyncLifecycleExecutor, BaseLifecycleExecutor)

    def test_async_executor_inherits_evaluator(self):
        assert issubclass(AsyncLifecycleExecutor, BaseLifecycleExecutor)

    def test_streaming_executor_inherits_evaluator(self):
        assert issubclass(StreamingLifecycleExecutor, BaseLifecycleExecutor)
