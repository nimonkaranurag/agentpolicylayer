...
class TestConditionEvaluator:
    ...
    def test_equals_direct_value(self):
        assert (
            self.evaluator.evaluate("hello", "hello")
            is True
        )
        assert (
    ...
    def test_gt(self):
        assert (
            self.evaluator.evaluate(10, {"gt": 5})
            is True
        )
        assert (
            self.evaluator.evaluate(5, {"gt": 5})
            is False
        )
        assert (
            self.evaluator.evaluate(None, {"gt": 5})
            is False
        )

    def test_gte(self):
        assert (
            self.evaluator.evaluate(5, {"gte": 5})
            is True
        )
        assert (
            self.evaluator.evaluate(4, {"gte": 5})
            is False
        )

    def test_lt(self):
        assert (
            self.evaluator.evaluate(3, {"lt": 5})
            is True
        )
        assert (
            self.evaluator.evaluate(5, {"lt": 5})
            is False
        )

    def test_lte(self):
        assert (
            self.evaluator.evaluate(5, {"lte": 5})
            is True
        )
        assert (
            self.evaluator.evaluate(6, {"lte": 5})
            is False
        )
    ...
    def test_not_negation(self):
        assert (
            self.evaluator.evaluate("a", {"not": "b"})
            is True
        )
        assert (
            self.evaluator.evaluate("a", {"not": "a"})
            is False
        )
    ...
    def test_none_condition(self):
        assert (
            self.evaluator.evaluate(None, None) is True
        )
        assert (
            self.evaluator.evaluate("x", None) is False
        )

    def test_custom_condition_registration(self):
        self.evaluator.register_condition(
            "starts_with",
            lambda val, prefix: isinstance(val, str)
            and val.startswith(prefix),
        )
        assert (
            self.evaluator.evaluate(
...
class TestObjectTraversal:
    ...
    def test_dict_traversal(self):
        obj = {"a": {"b": {"c": 42}}}
        assert (
            get_nested_value_by_dot_path(obj, "a.b.c")
            == 42
        )
    ...
    def test_metadata_traversal(
        self, sample_event: PolicyEvent
    ):
        result = get_nested_value_by_dot_path(
            sample_event, "metadata.user_region"
        )
        assert result == "EU"
    ...
    def test_single_level(self):
        assert (
            get_nested_value_by_dot_path({"x": 5}, "x")
            == 5
        )
...
class TestTemplateRenderer:
    ...
    def test_no_template_passthrough(self):
        assert (
            self.renderer.render("plain text", None)
            == "plain text"
        )
    ...
    def test_metadata_substitution(
        self, sample_event: PolicyEvent
    ):
        result = self.renderer.render(
            "User: {{metadata.user_id}}", sample_event
        )
...
class TestRuleEvaluator:
    ...
    def _make_event(
        self, output_text="test", user_region="EU"
    ):
        return PolicyEvent(
            id=str(uuid.uuid4()),
    ...
    def test_matching_rule_returns_verdict(self):
        ...
        result = (
            self.evaluator.evaluate_rule_against_event(
                rule, event
            )
        )
    ...
    def test_template_in_reasoning(self):
        ...
        assert (
            result.reasoning == "Blocked for EU users"
        )
    ...
    def test_multiple_conditions_all_must_match(self):
        ...
        event = self._make_event(
            output_text="hello world", user_region="EU"
        )
        assert (
            self.evaluator.evaluate_rule_against_event(
                rule, event
        ...
        event = self._make_event(
            output_text="hello world", user_region="US"
        )
        result = (
            self.evaluator.evaluate_rule_against_event(
                rule, event
...
class TestYAMLSchema:
    ...
    def test_yaml_manifest(self):
        m = YAMLManifest(
            name="server", version="1.0", policies=[]
        )
